...
public class Testing {
    ...
    public static void main(String[] args) throws IOException, InvalidMidiDataException, MidiUnavailableException, InterruptedException {
        ...
        rdb.addScore(new ChordProgression(Chord.fromOrdinals(0, 0), Chord.fromOrdinals(5, 0), Chord.fromOrdinals(7, 0), Chord.fromOrdinals(0, 0)), 1.0, 1.9);
        rdb.addScore(new ChordProgression(Chord.fromOrdinals(0, 0), Chord.fromOrdinals(7, 0), Chord.fromOrdinals(9, 0), Chord.fromOrdinals(5, 0)), 1.0, 1.9);
        rdb.addScore(new ChordProgression(Chord.fromOrdinals(9, 0), Chord.fromOrdinals(7, 0), Chord.fromOrdinals(5, 0), Chord.fromOrdinals(7, 0)), 1.0, 1.9);
        rdb.addScore(new ChordProgression(Chord.fromOrdinals(0, 0), Chord.fromOrdinals(5, 0), Chord.fromOrdinals(9, 0), Chord.fromOrdinals(7, 0)), 1.0, 1.9);
        System.out.println("c");
        //rdb.saveAs(new File("G:/chorddb/test2.db"));
        System.out.println("d");
        System.out.println("Starting genetic fit");
        GeneticFitter gf = new GeneticFitter(16, rdb.getFitnessFunction());
...
//        }
        ...
        for (Chord c : cp.chords) {
            ...
            trk.add(new MidiEvent(sm4, pos+180));
            trk.add(new MidiEvent(sm1, pos+0));
            trk.add(new MidiEvent(sm2, pos+60));
            trk.add(new MidiEvent(sm3, pos+120));
            ShortMessage smo1 = new ShortMessage();
            smo1.setMessage(NOTE_OFF, 60 + c.key.ordinal, 127);
            ShortMessage smo2 = new ShortMessage();
            smo2.setMessage(NOTE_OFF, (c.type == Chord.ChordType.Minor ? 63 : 64) + c.key.ordinal, 127);
            ...
            trk.add(new MidiEvent(smo4, pos + 240));
            trk.add(new MidiEvent(smo1, pos + 60));
            trk.add(new MidiEvent(smo2, pos + 120));
            trk.add(new MidiEvent(smo3, pos + 180));
            pos += 240;
        }
        ShortMessage sm1 = new ShortMessage();
